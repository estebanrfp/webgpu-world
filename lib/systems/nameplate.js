...
function renderNameplateToCanvasAnimated(ctx, name, status, accentColor, health, energy, xp, statusIcon) {
  ...
  const badges = ['🏆', '⭐', '🎯', '💎', '🔥'];
  // Show different number of badges based on entity (simulated achievements)
  const badgeCount = 1 + (name.charCodeAt(0) % 5); // 1-5 badges based on name
  const badgeY = panelY + barHeight + 32;
