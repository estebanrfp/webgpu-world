...
export function createNPCSystem(device, options = {}) {
  ...
  function moveTowards(npc, targetX, targetZ, speed, dt, playerX, playerZ, getTerrainHeight) {
    ...
    if (dist > 0.5) {
      ...
      // Rotate towards movement direction (instant rotation for natural walking)
      // Calculate the actual movement direction (after collision avoidance)
      const actualDx = newX - (npc.x - moveX - (newX - npc.x - moveX));
      const actualDz = newZ - (npc.z - moveZ - (newZ - npc.z - moveZ));
      
      // Use the original intended direction for rotation
        ...
        case 'following':
          {
            if (distToPlayer > stopDistance) {
              ...
              while (rotDiff > Math.PI) rotDiff -= Math.PI * 2;
              while (rotDiff < -Math.PI) rotDiff += Math.PI * 2;
              npc.rotation += rotDiff * 8 * dt;  // Faster rotation when standing
            }
          }
          break;
      }
