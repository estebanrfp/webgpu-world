...
export function createNPCSystem(device, options = {}) {
  ...
  async function init() {
    ...
    // Get skeleton and animations for creating independent NPC animation systems
    ...
    // Create NPC instances with independent animation systems
    console.log(`📍 Creating ${npcCount} NPCs with independent animations:`);
    ...
    console.log('✅ Each NPC will have independent animations!');
    
    for (let index = 0; index < npcCount; index++) {
      const angle = (index / npcCount) * Math.PI * 2 + (Math.random() - 0.5) * 0.5;
      const radius = 20 + Math.random() * 40;  // Spread out: 20-60 units from player
      ...
      // Create independent bone matrices buffer for this NPC
      const npcBoneMatricesBuffer = device.createBuffer({
        size: MAX_BONES * 16 * 4,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
      });

      // Create independent animation system for this NPC
      const npcAnimationSystem = new AnimationSystem(skeleton, animations);
      npcAnimationSystem.play('Idle');
      ...
      const npc = {
        ...
        // Independent bone matrices buffer for this NPC
        boneMatricesBuffer: npcBoneMatricesBuffer,
        // Independent animation system for this NPC
        animationSystem: npcAnimationSystem,
        ...
        state: Math.random() < 0.7 ? 'walking' : (Math.random() < 0.5 ? 'running' : 'idle'),
  ...
  function update(dt, playerX, playerY, playerZ, getTerrainHeight) {
    ...
    if (!hasSpawnedNearPlayer) {
      ...
      console.log(`📍 Spawning NPCs in the zone around player at (${playerX.toFixed(1)}, ${playerZ.toFixed(1)})...`);
    ...
    for (const npc of npcs) {
      ...
      if (npc.currentAnimation !== targetAnim) {
        npc.animationSystem.play(targetAnim);
      ...
      // Write bone matrices to NPC's buffer
