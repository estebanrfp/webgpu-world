...
// Use the same avatar as the player (guaranteed to work and can be cached)
// All NPCs will share the same model but with different behaviors
const NPC_AVATAR_URL = AVATAR_URL;
...
export function createNPCSystem(device, options = {}) {
  ...
  async function loadNPC(index, avatarUrl) {
    try {
      ...
      // Calculate spawn position (circle around player start position)
      const angle = (index / npcCount) * Math.PI * 2;  // Spread evenly in circle
      const radius = 8 + (index % 3) * 4;  // Close radius: 8, 12, 16 units
  ...
  async function init() {
    ...
    // All NPCs use the same avatar URL (efficient caching)
    const batchSize = 3;
    for (let i = 0; i < npcCount; i += batchSize) {
      const batch = [];
      for (let j = i; j < Math.min(i + batchSize, npcCount); j++) {
        batch.push(loadNPC(j, NPC_AVATAR_URL));
      }
  ...
  function updateUniforms(characterScale) {
    for (const npc of npcs) {
      ...
      const uniformData = new Float32Array([
        ...
        npc.x, npc.y, npc.z, 1,
        // Color (vec3f) + pad
        1.0, 1.0, 1.0, 0.0,  // White color, can be customized per NPC
      ]);
  ...
  function render(renderPass, characterPipeline) {
    if (!isLoaded) {
      console.log('NPC render: not loaded yet');
      return;
    }
    ...
    for (const npc of npcs) {
      if (!npc.bindGroup) {
        console.log(`NPC ${npc.id}: no bindGroup`);
        continue;
      }
    ...
    if (rendered === 0 && npcs.length > 0) {
      console.log(`NPC render: 0/${npcs.length} rendered (missing bindGroups)`);
    }
  }
