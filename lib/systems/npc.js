...
import { CONFIG, AnimationSystem } from '../index.js';
...
export function createNPCSystem(device, options = {}) {
  ...
  async function init() {
    ...
    // Instead of 50 AnimationSystems, we only create 3 (one per animation type)
    sharedAnimations = {
    ...
    console.log(`📍 Creating ${npcCount} NPCs with shared animation pools:`);
    ...
    console.log('✅ Using 3 shared animation pools (Idle, Walk, Run) - memory optimized!');
    ...
    for (let index = 0; index < npcCount; index++) {
      ...
      const initialState = Math.random() < 0.7 ? 'walking' : (Math.random() < 0.5 ? 'running' : 'idle');
      const initialAnim = initialState === 'running' ? 'Run' : (initialState === 'walking' ? 'Walk' : 'Idle');
  ...
  function update(dt, playerX, playerY, playerZ, getTerrainHeight) {
    ...
    if (!hasSpawnedNearPlayer) {
      ...
      for (let i = 0; i < npcs.length; i++) {
        const npc = npcs[i];
        const angle = (i / npcs.length) * Math.PI * 2 + (Math.random() - 0.5) * 0.3;
        const radius = 5 + Math.random() * 30;  // Closer: 5-35 units from player
    ...
    for (const npc of npcs) {
      ...
      let targetAnim = 'Idle';
      if (npc.state === 'walking' || (npc.state === 'following' && distToPlayer > stopDistance)) {
        targetAnim = 'Walk';
      } else if (npc.state === 'running') {
        targetAnim = 'Run';
      }
   ...
   * Bind groups are created per-animation type since NPCs share animation pools
   */
  ...
  function createBindGroups(characterLayout, shadowCharacterLayout, cameraBuffer, lightBuffer, shadowMapTexture, shadowMapSampler) {
    ...
    // Save layouts and buffers for dynamic bind group creation
    ...
    for (const npc of npcs) {
      ...
      const boneBuffer = sharedAnimations?.[npc.currentAnimation]?.buffer;
      if (!npc.texture || !npc.sampler || !boneBuffer) {
        console.error(`❌ NPC ${npc.id}: Missing resources - texture:${!!npc.texture}, sampler:${!!npc.sampler}, bones:${!!boneBuffer}`);
        continue;
      }

      // Create bind group for current animation
      npc.bindGroup = device.createBindGroup({
        ...
        entries: [
          ...
          { binding: 4, resource: { buffer: boneBuffer } },
      ...
      npc.lastAnimationForBindGroup = npc.currentAnimation;
    }
  ...
  function updateBindGroupsIfNeeded() {
    if (!savedLayouts || !savedBuffers || !sharedAnimations) return;
    
    const { characterLayout, shadowCharacterLayout } = savedLayouts;
    const { cameraBuffer, lightBuffer, shadowMapTexture, shadowMapSampler } = savedBuffers;
    
    for (const npc of npcs) {
      if (npc.lastAnimationForBindGroup === npc.currentAnimation) continue;
      if (!npc.uniformBuffer) continue;
      
      const boneBuffer = sharedAnimations[npc.currentAnimation]?.buffer;
      if (!boneBuffer) continue;
      
      // Recreate bind groups for new animation
      npc.bindGroup = device.createBindGroup({
        layout: characterLayout,
        entries: [
          { binding: 0, resource: { buffer: cameraBuffer } },
