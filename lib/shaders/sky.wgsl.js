...
export const skyShader = `
      ...
      @fragment fn fs(in: VSOut) -> @location(0) vec4f {
        ...
        let timeOfDay = fract(camera.time / cycleDuration);  // 0-1 through the day
        ...
        let sunSideFactor = smoothstep(-0.2, 0.6, sunAlignment);  // 0 = moon side, 1 = sun side
        ...
        let localDawnFactor = dawnFactor * sunSideFactor;  // Full dawn only on sun side
        let localDuskFactor = duskFactor * sunSideFactor;  // Full dusk only on sun side
        
        // Add night colors to moon side during dawn/dusk transitions
        // This creates the blue night sky on the moon's hemisphere
        let moonSideNightBoost = moonSideFactor * (dawnFactor + duskFactor);
        ...
        if (horizon < 0.0) {
          ...
          let moonSideReduction = smoothstep(0.3, 1.0, awayFromSun);  // Reduce orange on moon's side
          let warmAtmosphereReduction = mix(1.0, 0.15, moonSideReduction * (dawnFactor + duskFactor));  // 85% reduction on moon side during dawn/dusk
