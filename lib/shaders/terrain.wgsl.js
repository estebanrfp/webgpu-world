...
export const terrainShader = `
  ...
  fn getHeight(worldX: f32, worldZ: f32) -> f32 {
    ...
    var m = 0.0;
    m += smoothNoise(px, py) * 1.0;
    m += smoothNoise(px * 2.0, py * 2.0) * 0.5;
    m += smoothNoise(px * 4.0, py * 4.0) * 0.25;
    m += smoothNoise(px * 8.0, py * 8.0) * 0.125;
    m = m / 1.875;

    // Bounded island: a radial mask folds the procedural noise into ONE finite island centred on
    // the origin (land + mountains in the middle, sea all around) — the world is NOT infinite.
    // MUST stay identical across terrain.wgsl + shadow.wgsl + oceanShader.wgsl + math.js.
    let R = 500.0;
    let coast = (smoothNoise(px * 0.6 + 11.3, py * 0.6 + 7.1) - 0.5) * 60.0;
    let dist = sqrt(worldX * worldX + worldZ * worldZ) + coast;
    let nd = dist / R;
    // Gentle cone island: smooth slope from a central highland down through wide sandy beaches into
    // the sea. Noise adds inland hills/mountains but fades near the shore so the coast stays soft.
    let baseH = 46.0 * (1.0 - nd);
    let detail = m * 22.0 * smoothstep(1.1, 0.15, nd);
    let h = baseH + detail - smoothstep(1.0, 1.45, nd) * 30.0;
    return max(h, -40.0);
  }
  ...
  @fragment fn fs(in: VSOut) -> @location(0) vec4f {
    ...
    if (isUnderwater) {
      ...
      let causticsIntensity = pow(caustics, 0.7) * 0.7 * dayFactor;
